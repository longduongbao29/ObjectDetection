...
class ObjectDetection {
  ...
  static const String _modelPath =
      'assets/ssd_mobilenet.tflite'; // success
  static const String _labelPath = 'assets/ssd_mobilenet.txt';

  // static const String _modelPath =
  //     'assets/yolov5_dog.tflite'; // can process but not detect
  // static const String _labelPath = 'assets/yolov5_dog.txt';
  ...
  Future<void> _loadModel() async {
    ...
    _interpreter = await IsolateInterpreter.create(address: model.address);
  ...
  Uint8List convertCameraImageToUint8List(CameraImage cameraImage) {
    ...
    for (int i = 0; i < cameraImage.planes.length; i++) {
      planes.add(cameraImage.planes[i].bytes);
    }

    // Ghép các mảng dữ liệu từ các mặt phẳng thành một mảng Uint8List duy nhất
    Uint8List concatenatedBytes = Uint8List(cameraImage.width * cameraImage.height * 4); // Đây là một ước lượng, bạn có thể cần điều chỉnh nếu cần thiết
    int offset = 0;
    for (int i = 0; i < cameraImage.planes.length; i++) {
      concatenatedBytes.setRange(offset, offset + cameraImage.planes[i].bytes.length, cameraImage.planes[i].bytes);
  ...
  List<List<Object>> analyseImage(CameraImage cameraImage)  {
    log('Analysing image...');
    // Reading image bytes from file
    // final imageData =convertCameraImageToUint8List(cameraImage);
  ...
  List<List<Object>> _runInference(
    ...
    var output = List.filled(
        1, List.filled(10, List.filled(4, 0.0))); // Adjust the shape here

    // _interpreter!.runForMultipleInputs([input], output);
    log(input.shape.toString());
    log(output.shape.toString());
     _interpreter.run(input, output);

    return output.toList();
  }
  img.Image convertCameraImage(CameraImage image) {
    // Lấy các thông số chiều rộng và chiều cao của ảnh
    final int width = image.width;
    final int height = image.height;
    final int uvRowStride = image.planes[1].bytesPerRow;
    final int? uvPixelStride = image.planes[1].bytesPerPixel;
    // Tạo một List chứa dữ liệu YUV từ các planes
    final List<int> uvRowBytes = image.planes[1].bytes;
    final List<int> vuRowBytes = image.planes[2].bytes;

    // Tạo một List chứa dữ liệu của ảnh đã xử lý
    List<int> imageData = [];
    ...
    for (int x = 0; x < width; x++) {
      for (int y = 0; y < height; y++) {
        ...
        // Thêm các giá trị RGB vào imageData
        imageData.add(b);
        imageData.add(g);
        imageData.add(r);
      }
    ...
    // Tạo một đối tượng Image từ dữ liệu đã xử lý
    return img.Image.fromBytes(width : width, height:height,bytes: byteBuffer);
  }
